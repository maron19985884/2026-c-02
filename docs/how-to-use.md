...
1. [この雛形は何か](#1-この雛形は何か)
2. [役割分担](#2-役割分担)
3. [準備（最初の1回だけ）](#3-準備最初の1回だけ)
4. [要件定義書の書き方](#4-要件定義書の書き方)
5. [技術選定書の書き方](#5-技術選定書の書き方)
6. [開発の進め方](#6-開発の進め方)
7. [テストの進め方](#7-テストの進め方)
8. [承認の進め方（ウォーターフォールの場合）](#8-承認の進め方ウォーターフォールの場合)
9. [要件が変わったとき](#9-要件が変わったとき)
10. [設計書・テーブル定義書の作り方](#10-設計書テーブル定義書の作り方)
11. [困ったときは](#11-困ったときは)
...
#### ③ `CLAUDE.md` の命名規則・禁止事項を記入する

`CLAUDE.md` を開き、以下のセクションをプロジェクトに合わせて記入します。
...
- ログイン・会員管理
- 決済処理
（要件定義書の「対象外」セクションをここに転記する）
```

> **完了条件**: `CLAUDE.md` の命名規則とスコープ外機能が記入済みであること。
...
**技術選定書への記入・変更は人間のみが実施します。AI（Claude Code）は参照のみで、内容を追加・変更・削除してはなりません。**
...
フェーズ確定後に変更が必要な場合は、`/speckit.change` で変更要求書を通じて管理してください（§9 参照）。
