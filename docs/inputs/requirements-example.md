# 要件定義書 記入例（オンライン書店サンプル）

> **位置づけ**: これは `requirements.md` の**記入例**です。
> 実際のプロジェクトでは `requirements.md` に案件の内容を記入してください。
> 本ファイルは参照専用であり、AI・人間を問わず編集する必要はありません。
>
> 記入の手引きは [`docs/how-to-use.md`](../how-to-use.md) の「4. 要件定義書の書き方」を参照してください。

---

## 変更履歴

| バージョン | 日付 | 変更者 | 変更内容 |
|---|---|---|---|
| 1.0 | 2026-06-24 | h1nakamu | 初版（user_requirements.md を Spec Kit 形式に変換） |

---

## 1. 背景・目的

個人運営のオンライン書店において、ユーザーが書籍を閲覧し注文を完了できる購買フローを提供する。
学習・デモ用途のサンプルアプリケーションとして実装する。

---

## 2. 対象業務・スコープ

### 対象

- 書籍一覧の閲覧
- 書籍詳細の閲覧
- カートへの追加・数量変更・削除
- 注文情報（氏名・住所・メール）の入力と注文確定
- 注文完了の確認

### 対象外

- ログイン・会員管理
- 決済処理
- 在庫管理
- 管理画面
- レビュー・評価
- 検索・フィルター

---

## 3. 機能要件

| 要件ID | 画面 | 要件名 | 内容 | 優先度 |
|---|---|---|---|---|
| REQ-001 | 商品一覧 | 書籍一覧表示 | グリッド形式で販売中の書籍を表示する | 高 |
| REQ-002 | 商品一覧 | 書籍基本情報表示 | 書影・タイトル・著者・価格を一覧上で表示する | 高 |
| REQ-003 | 商品一覧 | 詳細遷移 | 書籍をクリックすると商品詳細画面へ遷移する | 高 |
| REQ-004 | 商品詳細 | 詳細情報表示 | 書影・タイトル・著者・価格・説明文を表示する | 高 |
| REQ-005 | 商品詳細 | カート追加 | 「カートに追加」ボタンで書籍をカートに追加する | 高 |
| REQ-006 | 商品詳細 | 一覧へ戻る | カート追加後も一覧画面へ戻れる | 高 |
| REQ-007 | カート | カート内容表示 | カート内の書名・単価・数量・小計を表示する | 高 |
| REQ-008 | カート | 数量変更 | 数量を増減でき、小計と合計がリアルタイムに更新される | 高 |
| REQ-009 | カート | 書籍削除 | 書籍をカートから削除でき、合計が即時更新される | 高 |
| REQ-010 | カート | 合計表示 | カート内全商品の合計金額を表示する（送料は対象外） | 高 |
| REQ-011 | カート | 注文フォームへ遷移 | 「注文手続きへ」ボタンで注文フォーム画面へ遷移する | 高 |
| REQ-012 | 注文フォーム | 顧客情報入力 | 氏名・住所・メールアドレスを入力できる（3項目すべて必須） | 高 |
| REQ-013 | 注文フォーム | バリデーション | 未入力・形式不正の場合はエラーメッセージを表示する | 高 |
| REQ-014 | 注文フォーム | 注文内容確認 | 注文商品と合計金額を同一画面で確認できる | 高 |
| REQ-015 | 注文フォーム | 注文確定 | 「注文する」ボタン1つで注文を確定し完了画面へ遷移する | 高 |
| REQ-016 | 注文完了 | 完了メッセージ | 注文完了メッセージを表示する | 高 |
| REQ-017 | 注文完了 | 注文番号表示 | 注文番号を画面上に表示する | 高 |
| REQ-018 | 注文完了 | 一覧へ戻る | 商品一覧へ戻るリンクを表示する | 高 |

---

## 4. 非機能要件

| 区分 | 要件 |
|---|---|
| 性能 | 特定なし（学習・デモ用途） |
| 可用性 | ローカル環境での動作 |
| セキュリティ | 認証情報を `.env` で管理し、コードへの直書きを禁止 |
| 保守性 | TypeScript で型安全性を確保する |

---

## 5. 制約条件

- Docker Compose を用いたコンテナ構成で動作すること
- フロントエンド port 3000、バックエンド port 4000 を使用すること

---

## 6. 画面遷移

```
商品一覧 → 商品詳細 → （カートに追加）
                              ↓
                           カート → 注文フォーム → 注文完了
                              ↑
                         （一覧から直接も可）
```

---

## 7. 用語定義

| 用語 | 定義 |
|---|---|
| 書籍 | 販売対象の商品。書影・タイトル・著者・価格・説明文を持つ |
| カート | ユーザーが購入予定の書籍を一時的に保持する仕組み |
| 注文番号 | 注文確定時にシステムが発行する一意の識別子 |
| 小計 | カート内の各書籍について「単価 × 数量」で算出した金額 |
| 合計 | カート内全書籍の小計の総和 |
