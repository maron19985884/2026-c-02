...
| `customerAddress` | string | ○ | 住所。空文字列不可 |
| `customerEmail` | string | ○ | メールアドレス。`local@domain`の基本構造が必要（research.md #5） |
...
```json
{ "error": "validation_error", "details": ["customerName is required", "customerEmail format is invalid"] }
```
