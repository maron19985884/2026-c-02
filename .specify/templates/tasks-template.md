...
> File paths below MUST match the actual Project Structure recorded in `plan.md`'s Structure Decision — do not default to a web app's `backend/`+`frontend/` layout unless `plan.md` actually describes one.
...
- [ ] T013 [US1] Implement [Component] in [path per plan.md Structure Decision]（`plan.md`の構造にプレゼンテーション層がある場合のみ発生）
